...
import random
from dataset_visual import InterviewDataset
from pathlib import Path
import wandb
import pandas as pd
import torch.nn.functional as F
...
def encode_article_context(tokenizer, raw_text, max_length) -> dict:
    ...
    if len(combined_tokens) > max_length:
        ...
        if mask_token_id not in combined_tokens:
            combined_tokens[-1] = mask_token_id  # Replace the last token with the mask token if the prev token too long
...
def retrieve_interview_segment(query_context, ground_truth_interview, interview_visual_embedding, interview_segments, bart, embedder, bart_tokenizer, fusion_moduel, device, MAX_LENGTH):

    mask_token = bart_tokenizer.mask_token
    if query_context[0].count(mask_token) != 1:
        raise AssertionError(f"Expected exactly 1 mask token in article_context_raw, but found {query_context[0].count(mask_token)} in: {query_context}")
    ...
    visual_embedding = torch.from_numpy(interview_visual_embedding).to(device)  
...
def retrieve_interview_segment_random(ground_truth_interview, interview_segments,recall_k):

    recallk_indices = random.sample(range(len(interview_segments)), recall_k)
    print("recallk_indices", recallk_indices)
    
    correct = 0
    for idx in recallk_indices:
        if fair_string_compare_ignore_case(interview_segments[idx], ground_truth_interview):
            correct = 1
            break
...
def calculate_sim_distribution(interview, interview_base, embedder, device):
    """
    how different is gt from other candidates
    """
    gt_embeddings = embedder.encode(interview_base, convert_to_tensor=True).to(device)  # Shape: (num_candidates, hidden_dim)
    query_embedding = embedder.encode(interview, convert_to_tensor=True).to(device)  # Shape: (1, hidden_dim)

    cosine_sim = F.cosine_similarity(query_embedding.unsqueeze(1), gt_embeddings.unsqueeze(0), dim=-1) # add batch size

    cosine_sim = cosine_sim.squeeze(0)  # Now shape: (num_candidates,)
    sorted_sim = torch.sort(cosine_sim, descending=True).values
    return sorted_sim
