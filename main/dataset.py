...
from main.config_hl import TVSUM_SPLITS, YOUTUBE_SPLITS, DOCUMENTARY_SPLITS
...
class DatasetHL(Dataset):
    def __init__(self,
        ...
        assert dset_name in ['tvsum', 'youtube', 'documentary']
        self.dset_name = dset_name
        # which dataset to use
        dset_domain = {'tvsum': TVSUM_SPLITS,
                       'youtube': YOUTUBE_SPLITS,
                       'documentary': DOCUMENTARY_SPLITS}
        ...
        self.v_feat_dirs = v_feat_dirs
        self.q_feat_type = "text_features"
        # self.q_feat_type = "last_hidden_state" # output from clip encoder
        self.q_feat_dir = t_feat_dir
        self.q_feat_dim = 512
        self.txt_drop_ratio = 0
        self.normalize_t = True
        self.normalize_v = True
        # list of object in annotation.jsonl files
        self.label = nncore.load(self.data_path)
        self.use_tef = use_tef
        # based on qid --> total number pairs
        if dset_name == 'tvsum' or dset_name == 'youtube':
            self.video_id = {
        ...
        if dset_name == 'documentary':
            # self.label is a list of dictionary: from jsonl files
            self.combined_dict = {key: value for d in self.label for key, value in d.items()} # combine into giant dict
            anno_keys  = list(self.combined_dict.keys()) # with annotation
            vid_feat_name = load_txt("/home/weihanx/videogpt/deepx_data7/vid_clip/vitname0720.txt")
            vit_keys = [vid for vid in anno_keys if vid.split('_')[0] in vid_feat_name]
            self.video_id = {
                k: [s for s in self.splits[domain][k] if s in vit_keys] # include annotation keys and video features
                for k in ('train', 'val')
            }
    ...
    def __getitem__(self, idx):
        ...
        # print(f"vid = {vid}, video = {video}, saliency = {saliency}") # checked
        ...
        elif self.dset_name == 'documentary':
            # randomly sample s_{p} >  0 as positive samples, and the rest as negative samples
            saliency_pos_labels = torch.Tensor([random.choice(torch.where(saliency > 0)[0].tolist())])
        else:
            raise NotImplementedError
        ...
        if self.use_tef:
            # temporal embedding
            ctx_l = video.shape[0]  # 1/(len of video)
            tef_st = torch.arange(0, ctx_l, 1.0) / ctx_l # temporal encoding feature start
            tef_ed = tef_st + 1.0 / ctx_l # temporal encoding feature end
            tef = torch.stack([tef_st, tef_ed], dim=1)  # (Lv, 2)
            # add temporal encoding to video frame features
    ...
    def _get_video_feat_by_vid(self, vid):
        ...
        for feat_type, _feat_dir in zip(self.v_feat_types, self.v_feat_dirs):
            ...
            # else:
            if True:
                vid = vid.split('_')[0] # videoname_queryname
                _feat_path = join(_feat_dir, f"{vid}.npz")
                _feat = np.load(_feat_path)["features"].astype(np.float32)
                if self.normalize_v:
                    _feat = l2_normalize_np_array(_feat)
    ...
    def _get_query_feat_by_qid(self, qid):
        ...
        if self.q_feat_type == "text_features":
            # q_feat = q_feat[:self.max_q_l]
            q_feat = q_feat
        if self.normalize_t:
            q_feat = l2_normalize_np_array(q_feat)
    ...
    def get_saliency(self, idx):
        if self.dset_name == 'tvsum':
            ...
            saliency = self.combined_dict[video_id]["saliency"]
    ...
    def evaluate(self, blob, k=5, save_dir=None, **kwargs): # blob save scores
        ...
        if self.dset_name == 'tvsum':
            ...
            for idx, score in enumerate(blob): # [batch1, batch2, batch3]
                saliency_scores = self.get_saliency(idx)
                valid_score = score[0][:len(saliency_scores)] # vide feature is padded, but I am comparing nonpaded here, one at a time

                inds = torch.argsort(valid_score, descending=True) # [1, 104]

                label = self.get_saliency(idx)[inds].tolist() # sort the label by descending order

                # num_gt = sum(label), also check whether num_gt is zero
                if (num_gt := sum(label)) == 0:
                    collected.append(0)
                    continue
                ...
                for i, gt in enumerate(label):
                    hits += gt # add one if gt is 1, add 0 if gt is 0
                    _rec = hits / num_gt # recall
                    _prc = hits / (i + 1) # precision
                    ap += (_rec - rec) * (prc + _prc) / 2 # average mean precision
                    rec, prc = _rec, _prc # update after each iteration
                collected.append(ap)
        else:
            raise NotImplementedError
...
def start_end_collate_hl(batch):
    ...
    batched_data = dict()
    for k in model_inputs_keys:
        # maxmium length in that batch, pad saliency score
...
if __name__ == '__main__':
    dataset_config = {'dset_name': 'documentary', 'domain': 'general', 'data_path': ['/home/weihanx/videogpt/deepx_data6/dataset/annotation.jsonl'], 'v_feat_types': 'clip', 'v_feat_dirs': '/home/weihanx/videogpt/deepx_data7/vid_clip', 't_feat_dir': '/home/weihanx/videogpt/deepx_data7/txt_clip', 'use_tef': True}
    dataset = DatasetHL(**dataset_config)
    idx = 0
    vid = dataset.get_video_id(idx)
    video = dataset._get_video_feat_by_vid(vid)
    saliency = dataset.get_saliency(idx)
