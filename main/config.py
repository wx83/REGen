...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--dset_name", type=str, choices=["qvhighlights", "charades", "anet", "tvsum", "youtube", "summe", "ego4d", "qfvs", "video2gif", "coin", "hacs", "vlp", "videocc", "tacos", "documentary"])
        ...
        parser.add_argument("--n_epoch", type=int, default=2, help="number of epochs to run")
...
def setup_model(opt):
    ...
    model, criterion = importer.build_model(opt) # build univtg model, load model and criterion
    ...
    optimizer = torch.optim.AdamW(param_dicts, lr=opt.lr, weight_decay=opt.wd) # only optimize trainable
    ...
    if opt.resume is not None: # load pretrained model
        logger.info(f"Load checkpoint from {opt.resume}")
        checkpoint = torch.load(opt.resume, map_location="cpu")
        # the pretrained model is using parallel
        # the keys in the state dictionary are automatically prefixed with 'module.', 
        # # reflecting the module structure used during training.
        # ensuring that the model’s parameters can be correctly mapped to the model architecture 
        # regardless of whether it was originally trained with parallelism or not.

        for key in list(checkpoint["model"].keys()):
            checkpoint["model"][key.replace('module.', '')] = checkpoint["model"].pop(key) # the save model contains module, but load don't need it
        model.load_state_dict(checkpoint["model"]) # load pretrained univtg model
