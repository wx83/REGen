...
def txt2clip(clip_model, text, output_file):
    # device_id = next(model.parameters()).device
    device_id = "cuda:0"
    encoded_texts = clip.tokenize(text).to(device_id)

    # print(f"clip_model = {clip_model.encode_text(encoded_texts)['last_hidden_state']}")
    text_feature = clip_model.encode_text(encoded_texts)['last_hidden_state']
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(args.save_dir, 'vid.npz')
    text = ""
    txt2clip(text, output_file)