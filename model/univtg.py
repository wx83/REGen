...
class WeightedPool(nn.Module):
    def __init__(self, dim):
        ...
        # trainable embedding W;
        self.weight = nn.Parameter(weight, requires_grad=True)

    def forward(self, x, mask):
        alpha = torch.tensordot(x, self.weight, dims=1)  # shape = (batch_size, seq_length, 1)
        alpha = mask_logits(alpha, mask=mask.unsqueeze(2)) # mask: (batch_size, seq_length), alpha: (batch_size, seq_length, 1)
        alphas = nn.Softmax(dim=1)(alpha) # A = softmax(WQ), apply softmax on seq dimension
...
class Model(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        Parameters:
            ...
            # foreground: of primary interest
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_cls=None, src_cls_mask=None):
        bs = src_vid.shape[0]
        # project text and video data into the same space:  aligning the features from different modalities
        src_vid = self.input_vid_proj(src_vid)
        src_txt = self.input_txt_proj(src_txt) 
        if src_cls is not None: # class information, not used in pretrained model
            src_cls = self.input_txt_proj(src_cls)
        device_id = src_vid.device

        # type token. 0: video, 1: text, distinguish type of information
        ...
        memory = self.transformer(src, ~mask, pos) # (L, batch_size, d):  represents a high-level encoded representation of the concatenated inputs
        vid_mem = memory[:, :src_vid.shape[1], :]  # (bsz, L_vid, d): take the output from multimodal transformer and perform prediction
        # class and spans
        outputs_class = self.class_embed(vid_mem).sigmoid()  # (#layers, batch_size, #queries, #classes): foreground head
        outputs_coord = self.span_embed(vid_mem)  # (#layers, bsz, #queries, 2 or max_v_l * 2) # bounday head, left and right offsets
        ...
        # outputs_class: 0: background, 1: foreground
...
class SetCriterion(nn.Module):
    ...
    def loss_saliency(self, outputs, targets, indices, log=True):
        ...
        # print(f"outpus = {outputs['saliency_scores'].shape}, target = {saliency_scores.shape}")
        if saliency_scores.sum() == 0:
            return {"loss_s_inter": 0., "loss_s_intra": 0.}

        # * inter-vid mode
        vid_mem_proj = outputs["vid_mem_proj"] # max length of the video
        # random sample pos, if < 0, then it is negative sample
        pos_indices = targets["saliency_pos_labels"][:,0].long()  # (N, #pairs)
        batch_indices = torch.arange(len(vid_mem_proj)).to(vid_mem_proj.device)

        vid_feats = vid_mem_proj[batch_indices, pos_indices] # positive video feature
        ...
        # * intra-vid mode
        mask = targets['timestamp_mask'] # mask those padded video features
        selected_scores = saliency_scores[batch_indices, pos_indices].unsqueeze(-1) # target saliency scores
...
if __name__ == "__main__":
    ...
    # Load the checkpoint
    checkpoint_path = '/home/weihanx/videogpt/workspace/UniVTG/results/model_best.ckpt'
    checkpoint = torch.load(checkpoint_path)

    # Assuming you have a model class and the model is instantiated
    # Replace YourModelClass with the actual name of your model class
    model = Model()  # Instantiate your model
    model.load_state_dict(checkpoint['state_dict'])  # Load the weights from the checkpoint

    # Access the weights of the WeightedPool layer
    # Assuming your WeightedPool layer is named 'weighted_pool' in the model
    weight_pool_weights = model.weighted_pool.weight.data

    # Save the weights to a file
    torch.save(weight_pool_weights, 'weighted_pool_weights.pth')

    print("Weighted pool layer weights saved successfully.")